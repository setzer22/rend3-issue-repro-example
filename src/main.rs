...
use rend3::Renderer;
use rend3_gltf::{GltfLoadError, LoadedGltfScene};
...
fn load_model(renderer: &Renderer) -> Result<LoadedGltfScene, GltfLoadError<std::io::Error>> {
    let path = std::path::Path::new("assets/Footman_RIG.glb");
    let gltf_data = std::fs::read(&path).expect("Could load asset");
    let parent_directory = path.parent().expect("Has parent");
    pollster::block_on(rend3_gltf::load_gltf(&renderer, &gltf_data, |p| {
        rend3_gltf::filesystem_io_func(&parent_directory, p)
    }))
}
...
fn main() {
    ...
    let mut tonemapping_routine = rend3_routine::TonemappingRoutine::new(
        &renderer,
        render_texture_options.resolution,
        format,
    );

    let model_handle = load_model(&renderer).unwrap();
